...
export function DeliveryDayClient({
  ...
  function handleAddSale() {
    ...
    if (!amt || amt <= 0) {
      setSaleError("Ingresá un total válido.");
      return;
    }
    if (saleMode === "fixed" && !selectedClient) {
      setSaleError("Seleccioná un cliente.");
      return;
    }
    if (saleMode === "temp" && !tempName.trim()) {
      setSaleError("Escribí el nombre del cliente.");
      return;
    }
  ...
  function handleAddClient() {
    ...
    if (!newClientName.trim()) {
      setClientError("Escribí un nombre.");
      return;
    }
    startTransition(async () => {
      const result = await createDeliveryClientAction({
  ...
  function handleAddExpense() {
    ...
    if (!expenseName.trim()) {
      setExpenseError("Escribí el nombre del gasto.");
      return;
    }
    if (!amt || amt <= 0) {
          ...
          <div className="card-body">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-bold tabular-nums text-amber-700 sm:text-4xl">
                  {formatCurrency(totalSales)}
                </p>
                <p className="mt-1.5 text-xs text-stone-400">
                  {sales.length} {sales.length === 1 ? "entrega" : "entregas"}
                </p>
              </div>
          ...
          <div className="card-body">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-bold tabular-nums text-red-600 sm:text-4xl">
          ...
          <div className="card-body">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p
                  className={`mt-2 text-3xl font-bold tabular-nums sm:text-4xl ${isPositive ? "text-green-700" : "text-red-600"}`}
                >
              ...
              <div
                className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${isPositive ? "bg-green-50" : "bg-red-50"}`}
              >
                <Wallet
                  className={`size-5 ${isPositive ? "text-green-600" : "text-red-500"}`}
                />
              </div>
            </div>
          </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {sales.map((sale) => (
                  <tr
                    key={sale.id}
                    className={sale.id.startsWith("temp-") ? "opacity-50" : ""}
                  >
                    <td>
                      <div className="flex items-center gap-2">
                        <span
                        ...
                        {sale.isTemp && (
                          <span className="rounded-full bg-stone-100 px-1.5 py-0.5 text-xs text-stone-400">
                            temporal
                          </span>
                        )}
                      </div>
                    ...
                    <td className="text-center tabular-nums font-semibold text-amber-700">
                      {sale.notes ?? "—"}
                    </td>
              ...
              <tfoot>
                <tr className="border-t-2 border-stone-200 bg-amber-50/50">
                  <td
                    colSpan={2}
                    className="px-4 py-3 text-sm font-semibold text-stone-700"
                  >
                    Total ventas
                  </td>
                  <td className="px-4 py-3 text-right sm:text-left tabular-nums text-lg font-bold text-amber-700">
                    {formatCurrency(totalSales)}
                  </td>
                  <td colSpan={2} />
                </tr>
            ...
            <table className="data-table">
              ...
              <tbody>
                {expenses.map((exp) => (
                  <tr
                    key={exp.id}
                    className={exp.id.startsWith("temp-") ? "opacity-50" : ""}
                  >
                    <td className="font-medium">{exp.name}</td>
                    <td className="tabular-nums text-xs text-stone-400">
                      {formatDateTime(exp.createdAt)}
                    </td>
                    <td className="text-start tabular-nums font-semibold text-red-600">
              ...
              <tfoot>
                <tr className="border-t-2 border-stone-200 bg-red-50/50">
                  ...
                  <td />
                  <td className="px-4 py-3 text-right tabular-nums text-lg font-bold text-red-600">
                    {formatCurrency(totalExpenses)}
                  </td>
                  <td />
                </tr>
            ...
            <>
              ...
              {saleAmt > 0 && (
                <div className="flex items-center justify-between rounded-xl border border-amber-100 bg-amber-50 p-3">
                  ...
                  <span className="tabular-nums text-xl font-bold text-amber-700">
