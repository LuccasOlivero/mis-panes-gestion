...
export async function getDeliveryClientsAction(): Promise<
  Result<DeliveryClient[]>
> {
  try {
...
export async function getDailyDeliverySummaryAction(
  ...
  try {
    ...
    const sales = (salesRes.data ?? []).map((r) =>
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      mapSale(r, (r as any).delivery_clients?.name),
    );
...
export async function createDeliverySaleAction(
  input: CreateDeliverySaleInput,
): Promise<Ok> {
  try {
...
export async function createDeliveryExpenseAction(
  input: CreateDeliveryExpenseInput,
): Promise<Ok> {
  try {
    if (!input.name.trim())
...
export async function deleteDeliveryExpenseAction(
  expenseId: string,
): Promise<Ok> {
  try {
    const supabase = createServerClient();
...
export async function getDeliveryNotesAction(): Promise<
  Result<DeliveryNote[]>
> {
  try {
    const supabase = createServerClient();
...
export async function createDeliveryNoteAction(
  ...
  try {
    ...
    const { error } = await supabase
      .from("delivery_notes")
      .insert({ content: input.content.trim() });
    if (error) throw new Error(error.message);
    revalidatePath("/reparto");
    return { success: true };
  } catch (e) {
...
export async function getDeliveryOrdersAction(): Promise<
  Result<{
    pending: DeliveryOrder[];
    completed: DeliveryOrder[];
    urgent: DeliveryOrder[];
  }>
> {
  try {
    const supabase = createServerClient();
...
export async function markOrderDeliveredAction(orderId: string): Promise<Ok> {
  try {
    ...
    const { data: order, error: fetchErr } = await supabase
      .from("delivery_orders")
      .select(
        "id, client_name, description, delivery_date, sale_amount, sale_created, completed",
      )
      .eq("id", orderId)
      .single();
...
export async function getUrgentOrdersAction(): Promise<
...
> {
  try {
    ...
    const today = format(todayAR, "yyyy-MM-dd");
    const twoDaysAhead = format(
      new Date(todayAR.getTime() + 2 * 24 * 60 * 60 * 1000),
      "yyyy-MM-dd",
    );
    const { data, error } = await supabase
      .from("delivery_orders")
